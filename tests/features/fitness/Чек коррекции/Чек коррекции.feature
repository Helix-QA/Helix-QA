...
Сценарий: Создания продажи	
	...
	*Проверка страници "Создание чака коррекции"
		И элемент формы с именем 'АвторДокумента' стал равен "Админ"
		И элемент формы с именем 'АдресСайта' стал равен ""
		...
		И элемент формы с именем 'ДополнительныйРеквизит' стал равен ""
		...
		И таблица 'ПозицииЧека' стала равной по шаблону:
			...
			| '1' | '250,00'            | '$$УслугаГрупповая$$'           | '1,000'      | ''             | '250,00'           | ''             | 'Услуга'                   | 'Без НДС'    | ''          | ''         | 'Передача с полной оплатой' | ''                                   | ''             | ''    | ''        |
	...
	*Проверка страници "Исходные данные чека"
		Тогда элемент формы с именем 'XML' стал равен по шаблону 
			...
			|'	</Parameters>'|
			|'	<Positions>'|
			|'		<FiscalString Name=\" $$УслугаГрупповая$$\" Quantity=\"1\" PriceWithDiscount=\"250\" AmountWithDiscount=\"250\" DiscountAmount=\"0\" Department=\"0\" VATRate=\"none\" PaymentMethod=\"4\" CalculationSubject=\"4\" MeasureOfQuantity=\"0\">'|
			|'			<IndustryAttribute/>'|
			|'			<AgentData/>'|
			|'			<VendorData/>'|
			|'		</FiscalString>'|
		...
		И таблица 'ПозицииЧека' стала равной:
			...
			| ' $$УслугаГрупповая$$' | '1,00'       | ''             | ''     | '250,00'           | '250,00' | ''             | 'Услуга'                   | ''           | ''          | ''         | 'Передача с полной оплатой' |
