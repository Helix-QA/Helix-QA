...
*Создание продажи
	...
	*Проверка json
		...
		И таблица 'Список' содержит строки по шаблону:
			| 'Дата регистрации заказа'                        | 'Клиент'      | 'Платежный шлюз' | 'Эквайринговый терминал'                   | 'Дата действия заказа до'                        | 'Источник записи'  | '№ заказа в 1С' | '№ заказа в банке' | 'Документ'                                                   | 'Статус оплаты в банке'                | 'Статус в 1С' | 'Сумма чека' | 'Структурная единица' | 'Параметры JSON'                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
			| '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:*:*' | '$$Фамилия$$' | 'ЮКасса'         | '03.Юкасса статический QR-код (эквайринг)' | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:*:*' | 'Ссылка на оплату' | '*'             | ''                 | 'Продажа * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:* ' | 'Заказ зарегистрирован, но не оплачен' | 'Не оплачено' | '3 000,00'   | 'Фитнес плюс'         | '{"amount":{"value":"3000","currency":"RUB"},"capture":true,"description":"Продажа *, $$Фамилия$$, тел.: +7 (*) *, email: *","save_payment_method":false,"confirmation":{"type":"redirect","return_url":"https://stubohub.front-main.acqr1.cloud.yookassa.ru/sbpQrPay"},"receipt":{"customer":{"phone":"*","email":"*"},"items":[{"description":"$$НоменклатураЮкассы$$","quantity":"1.00","amount":{"value":"3000","currency":"RUB"},"vat_code":1,"payment_mode":"full_payment","payment_subject":"service"}],"tax_system_code":1},"metadata":{"cms_name":"fitness1c","ClubID":"f35ca849-23fa-11f0-bbe0-0050568bbe81","ClubName":"Фитнес плюс","customerNumber":"$$Фамилия$$","orderNumber":"*"}}' |
...
//		И я открываю внешнюю обработку или отчет "D:\Репозиторий(HELIX)\Helix-QA\tests\epf\ТестОплатПоЮКассеТабличка.epf" (Расширение)
		...
		И у элемента формы с именем 'ДатаСозданияЗаказа' текст редактирования стал равен '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")}'
	...
	*Проверка json
		...
		И таблица 'Список' стала равной:
			| 'Дата регистрации заказа'                        | 'Клиент'      | 'Платежный шлюз' | 'Эквайринговый терминал'                   | 'Дата действия заказа до'                        | 'Источник записи'  | '№ заказа в 1С' | '№ заказа в банке' | 'Документ'                                                   | 'Статус оплаты в банке' | 'Статус в 1С' | 'Сумма чека' | 'Структурная единица' | 'Параметры JSON'                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
			| '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:*:*' | '$$Фамилия$$' | 'ЮКасса'         | '03.Юкасса статический QR-код (эквайринг)' | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:*:*' | 'Ссылка на оплату' | '$НомерЗаказа$' | '*'                | 'Продажа * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:* ' | 'Заказ успешно оплачен' | 'Оплачено'    | '3 000,00'   | 'Фитнес плюс'         | '{\n  "id" : "*",\n  "status" : "succeeded",\n  "amount" : {\n    "value" : "3000.00",\n    "currency" : "RUB"\n  },\n  "income_amount" : {\n    "value" : "3000.00",\n    "currency" : "RUB"\n  },\n  "description" : "Продажа *, $$Фамилия$$, тел.: +7 (*) *, email: *",\n  "recipient" : {\n    "account_id" : "67190",\n    "gateway_id" : "35278211"\n  },\n  "payment_method" : {\n    "type" : "sbp",\n    "id" : "*",\n    "saved" : false,\n    "status" : "inactive",\n    "sbp_operation_id" : "*",\n    "payer_bank_details" : {\n      "bank_id" : "*",\n      "bic" : "*"\n    },\n    "qrc_id" : "$GQR0PNO7DMXP9KZ2M81VONSSI3IBX6GH$",\n    "params_id" : "*"\n  },\n  "captured_at" : "*-*-*:*:*.*",\n  "created_at" : "*-*-*:*:*.*",\n  "test" : false,\n  "refunded_amount" : {\n    "value" : "0.00",\n    "currency" : "RUB"\n  },\n  "paid" : true,\n  "refundable" : true,\n  "metadata" : {\n    "ClubName" : "Фитнес плюс",\n    "orderNumber" : "$НомерЗаказа$",\n    "ClubID" : "f35ca849-23fa-11f0-bbe0-0050568bbe81",\n    "customerNumber" : "$$Фамилия$$",\n    "cms_name" : "fitness1c"\n  }\n}' |