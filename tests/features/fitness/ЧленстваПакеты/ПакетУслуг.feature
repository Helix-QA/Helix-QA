		...
		*Проверка в персональном занятии
			...
			И я выполняю код встроенного языка
				"""bsl
					...
					ЧастиСтроки = СтрРазделить(Контекст.ТекущаяДата, ".");
					// Преобразуем в числа
					День = Число(ЧастиСтроки[0]);
					Месяц = Число(ЧастиСтроки[1]);
					Год = Число(ЧастиСтроки[2]);
					// Создаем объект Дата
					ДатаОбъекта = Дата(Год, Месяц, День);
					// Получаем конец месяца
					ДатаОкончанияМесяца = КонецМесяца(ДатаОбъекта);
					// Преобразуем в строку нужного формата
					СтрокаОкончания = Формат(ДатаОкончанияМесяца, "ДФ=dd.MM.yyyy");
					// Записываем результат в контекст как строку
					Контекст.Вставить("КонецЧленства", СтрокаОкончания);
				"""
			И я нажимаю кнопку выбора у поля с именем 'ПолеОснованияОплаты_0'
	...
	*Создание продажи и проверка создания и зачисления средств на лицевой счет
		...
		*Проверка на создание нового лицевого счета
			...
			И таблица 'Список' содержит строки по шаблону:
				...
				| 'Пакет услуг №*' | ''   | 'Тестовый клиент' | 'Открыт' | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:*:*' | ''  | '1 000'   |
