...
//			"""
	...
	*Проверка элементов формы справки в налоговую.
		...
		И таблица 'Занятия' стала равной по шаблону:
			| 'N' | 'Продажа'                         | 'Номенклатура'           | 'Сумма'    |
			| '1' | 'Продажа * от $ТекущаяДата$ *:* ' | '$$ПакетУслуг$$'         | '2 000,00' |
			| '2' | 'Продажа * от $ТекущаяДата$ *:* ' | '$$УслугаГрупповая$$'    | '250,00'   |
			| '3' | 'Продажа * от $ТекущаяДата$ *:* ' | '$$УслугаПерсональная$$' | '250,00'   |
			| '4' | 'Продажа * от $ТекущаяДата$ *:* ' | '$$Членство$$'           | '555,00'   |
			| '5' | 'Продажа * от $ТекущаяДата$ *:* ' | '$$Членство$$'           | '555,00'   |
			| '6' | 'Продажа * от $ТекущаяДата$ *:* ' | '$$ПакетУслуг$$'         | '2 000,00' |
			| '7' | 'Продажа * от $ТекущаяДата$ *:* ' | '$$УслугаПерсональная$$' | '250,00'   |
			| '8' | 'Продажа * от $ТекущаяДата$ *:* ' | '$$УслугаГрупповая$$'    | '250,00'   |
