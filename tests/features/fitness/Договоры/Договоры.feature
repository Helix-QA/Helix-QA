...
//		И я выбираю файл '$КаталогПроекта$\tests\features\fitness\Договоры\pngtree-fitness-logo-image_81956.jpg'
		И я выбираю файл '$КаталогПроекта$\fitness\Договоры\pngtree-fitness-logo-image_81956.jpg'
...
Сценарий: Я заполняю документ в Внешний файл
	...
	Тогда открылось окно "Бланки договоров"
	И в таблице 'Список' я перехожу к строке:
		| "Вид бланка" | "Наименование" |
		| "Договор"    | "Внешний файл" |
	И в таблице 'Список' я выбираю текущую строку
	Тогда открылось окно "Внешний файл (Бланки договоров)"
...
//	И я выбираю файл '$КаталогПроекта$\tests\features\fitness\Договоры\Договор.docx'
	И я выбираю файл '$КаталогПроекта$\fitness\Договоры\Договор.docx'
...
Сценарий: Я Проверяю на правильность и заполнения HTML шаблон
	...
	*Заполнение полей: префикс, номер, дата, членства, клиент.
		...
		И в дополнение формы с именем 'СтрокаПоиска' я ввожу текст "1234"
		И я жду, что в таблице "Список" количество строк будет "больше" 0 в течение 20 секунд
		И я перехожу к следующему реквизиту
		И в таблице 'Список' я выбираю текущую строку			
...
Сценарий: Я Проверяю на правильность и заполнения Внешний файл
	...
	*Заполнение полей: префикс, номер, дата, членства, клиент.
		...
		И я нажимаю кнопку выбора у поля с именем 'ЧленствоПакетУслуг'
		И я активизирую дополнение формы с именем 'СтрокаПоиска'
		И в дополнение формы с именем 'СтрокаПоиска' я ввожу текст "Активен"
		И я жду, что в таблице "Список" количество строк будет "больше" 0 в течение 20 секунд
		И я перехожу к следующему реквизиту
		И я перехожу к закладке с именем 'МенюФильтрацииДанных'
		И в таблице 'Список' я выбираю текущую строку				
