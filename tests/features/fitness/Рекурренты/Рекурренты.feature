...
Сценарий: Создание платежной связки и настройка оборудования.
	...
	*Настройка SRP
		...
		И я перехожу к следующему реквизиту
		И в поле с именем 'ВремяОкончанияОтправкиПлатежейВБанк' я ввожу текст "$ВремяОтправкиПлатежей$"
	...
	*Смена дат в договоре и членстве на прошлый месяц	
		...
		*Смена даты активации в вступительном членстве
		...
		//Запоминаем рандомное число от "40" до "60", чтобы определить точное кол-во дней заморозки
			И я запоминаю случайное число в диапазоне от "40" до "60" в переменную "КолВоДнейЗаморозки"
		...
		*Вычисляем разницу дней между текущей датой и датой заморозки
			...
			И Я запоминаю значение выражения 'Число(60)- Число($ДопСрокДействия$)' в переменную "ОстатокЗаморозок"
...
Сценарий: Проверка автопродления членства с автоматической оплатой по реестру рекуррентных членств
	...
	*Создание нового реестра рек.платежей и проверка автопродления
		...
		И таблица 'РекуррентныеПлатежи' содержит строки по шаблону:
			...
			| '1' | '$$Фамилия$$' | ''                     | ''                | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")}' | 'Нет'                  | ''     | '1'             | 'Рек. договор: * от *.*.* (Активен)' | '****************привязка31.10.202516:03' | '1'  | 'Ожидание отправки в банк' | ''           | ''            | ''      |
		...
		И таблица 'РекуррентныеПлатежи' содержит строки по шаблону:
			| 'N' | 'Клиент'      | 'Идентификатор заказа' | 'Документ оплаты'                                | 'Дата платежа (план)'                      | 'Оплачивается вручную' | 'Дата'        | 'Номер попытки' | 'Договор'                            | 'Номер карты'                             | ' №' | 'Статус'             | 'Реализация'                                                 | 'Комментарий' | 'Сумма'    |
			| '1' | '$$Фамилия$$' | '*'                    | 'Оплата от клиента платежной картой * от *.*.* ' | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")}' | 'Нет'                  | '*.*.* *:*:*' | '1'             | 'Рек. договор: * от *.*.* (Активен)' | '****************привязка31.10.202516:03' | '1'  | 'Подтвержден банком' | 'Продажа * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:* ' | 'succeeded'   | '1 500,00' |
...
Сценарий: Проверка автопродления членства с ручной оплатой по реестру рекуррентных членств
	...
	*Создание нового реестра рек.платежей и проверка автопродления
		...
		И таблица 'РекуррентныеПлатежи' содержит строки по шаблону:
			| 'N' | 'Клиент'      | 'Идентификатор заказа' | 'Документ оплаты' | 'Дата платежа (план)'                      | 'Оплачивается вручную' | 'Дата' | 'Номер попытки' | 'Договор'                                                               | 'Номер карты'                             | ' №' | 'Статус'                 | 'Реализация' | 'Комментарий' | 'Сумма' |
			| '1' | '$$Фамилия$$' | ''                     | ''                | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")}' | 'Да'                   | ''     | '1'             | 'Рек. договор: * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} (Активен)' | '****************привязка31.10.202516:03' | '1'  | 'Ожидание ручной оплаты' | ''           | ''            | ''      |
		И я нажимаю на кнопку с именем 'КнопкаЗаписать'
		И я сохраняю навигационную ссылку текущего окна в переменную "Реестр"		
		И я нажимаю на кнопку с именем 'ФормаКнопкаПровестиИЗакрыть'
		И я закрываю все окна клиентского приложения
		...
		И таблица 'РекуррентныеПлатежи' содержит строки по шаблону:
			| 'N' | 'Клиент'      | 'Идентификатор заказа' | 'Документ оплаты' | 'Дата платежа (план)'                      | 'Оплачивается вручную' | 'Дата' | 'Номер попытки' | 'Договор'                                                               | 'Номер карты'                             | ' №' | 'Статус'                 | 'Реализация'                                                 | 'Комментарий' | 'Сумма'    |
			| '1' | '$$Фамилия$$' | ''                     | ''                | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")}' | 'Да'                   | ''     | '1'             | 'Рек. договор: * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} (Активен)' | '****************привязка31.10.202516:03' | '1'  | 'Ожидание ручной оплаты' | 'Продажа * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:* ' | ''            | '1 500,00' |
...
*Открытие договора рек.платежей и проверка дат в нем
	И я выполняю код встроенного языка
		"""bsl
			...
			КонецСледующегоМесяца = КонецМесяца(ДобавитьМесяц(ТекДата, 1));

			// Форматируем даты
			ФорматированнаяКонецТекущего = Формат(КонецТекущегоМесяца, "ДФ=дд.ММ.гггг");
			ФорматированнаяКонецСледующего = Формат(КонецСледующегоМесяца, "ДФ=дд.ММ.гггг");

			// Сохраняем в контекст
			Контекст.Вставить("КонецТекущегоМесяца", ФорматированнаяКонецТекущего);
