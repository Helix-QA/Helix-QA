...
*Основная информация
	...
	И я запоминаю случайное число в переменную "СЛУЧСЛО"
	И я запоминаю значение выражения '"Фамилия" + Формат($СЛУЧСЛО$, "ЧГ=0")' в переменную "$$ФамилияСотрудника$$"
	И в поле с именем 'Фамилия' я ввожу текст '$$ФамилияСотрудника$$'

	И я запоминаю случайное число в переменную "СЛУЧСЛО"
	И я запоминаю значение выражения '"Имя" + Формат($СЛУЧСЛО$, "ЧГ=0")' в переменную "$$ИмяСотрудника$$"
	И в поле с именем 'Имя' я ввожу текст '$$ИмяСотрудника$$'

	И я запоминаю случайное число в переменную "СЛУЧСЛО"
	И я запоминаю значение выражения '"Отчество" + Формат($СЛУЧСЛО$, "ЧГ=0")' в переменную "$$ОтчествоСотрудника$$"
...
*Приложение тренера
	...
	Тогда открылось окно "Сегменты номенклатуры"
	И я нажимаю на кнопку с именем 'Создать'
	Тогда открылось окно "Сегмент номенклатуры (создание)"
...
*Фото		
	И я выбираю файл "C:\GitHab\Helix-QA\tests\features\fitness\Сотрудники\12.jpg"
...
*Помещение сотрудника
	...
	Тогда открылось окно "Помещения"
	И я нажимаю на кнопку с именем 'Создать'
	Тогда открылось окно "Помещение (создание)"
	И в поле с именем 'Наименование' я ввожу текст "12313213"
	И я изменяю флаг с именем 'ИспользоватьВРасписанииЗанятий'
	И я изменяю флаг с именем 'МониторингСвободногоПосещения'
	И я изменяю флаг с именем 'ОграничениеНаполнения'
	И я изменяю флаг с именем 'НеКонтролироватьЗанятостьСотрудника'
	И я изменяю флаг с именем 'НеКонтролироватьЗанятостьПомещения'
	И в поле с именем 'ОдновременноКлиентовВКлубе' я ввожу текст "1 231"
	И в поле с именем 'ДлительностьСлотаПосещения' я ввожу текст "123"
	И я нажимаю на кнопку с именем 'КнопкаДополнительно'
	И из выпадающего списка с именем 'СтруктурнаяЕдиница' я выбираю по строке ""
	И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'
	Тогда открылось окно "Помещения"
	И я нажимаю на кнопку с именем 'КнопкаВыбрать'
...
*Ограничение по помещениям
	Когда открылось окно "Фамилия700723571 Имя3333063701 Отчество3465956099 (Сотрудник) *"
	И я устанавливаю флаг с именем 'ОграничениеПоПомещениям'
	И я нажимаю на кнопку с именем 'УстановитьОграничениеПоПомещениям'
	Тогда открылось окно "Выбор доступных помещений сотрудника"
...
*Проверка Переключение между вкладками в карточке сотрудника
	...
	И я запоминаю случайное число в переменную "СЛУЧСЛО"
	И я запоминаю значение выражения '"Наименование" + Формат($СЛУЧСЛО$, "ЧГ=0")' в переменную "$$НаименованиеСотрудника$$"
	...
	И я нажимаю на кнопку с именем 'Создать'
	Тогда открылось окно "Сегмент сотрудников (создание)"
	Когда открылось окно "Сегмент сотрудников (создание)"
...
*Проверка заполнения графика работы с использованием мастера заполнения и в ручном варианте, а так-же отправка на печать
	...
	И в таблице 'ИнтервалыВремени' из выпадающего списка с именем 'ИнтервалыВремениКонец' я выбираю точное значение "21:00"
...
*Проверка кнопки создать и скопировать. Только на то что после нажатия форма открывается, потом можно ее сразу закрыть.	
	...
	Тогда открылось окно "Начисления и удержания (создание)"
...
*Загрузка файлов//
	И я выбираю файл 'C:\GitHab\Helix-QA\tests\features\fitness\Сотрудники\2830514.png'
	И В текущем окне я нажимаю кнопку командного интерфейса "Файлы"
		
*Сохранение
	И В текущем окне я нажимаю кнопку командного интерфейса "Основное"
	И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'
	Тогда открылось окно "Данные были изменены"
