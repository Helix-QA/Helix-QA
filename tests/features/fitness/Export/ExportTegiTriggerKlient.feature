	...
	*Проверка табличной части
		Если переменная "Наименование" имеет значение "Онлайн регистрация клиента $$СлучайноеЧисло$$" Тогда
			Тогда таблица "Список" содержит строки по шаблону:
				| 'Триггерное событие' | 'Действие (описание)'         | 'Статус события'   | 'Период'        | 'Часовой пояс'  | 'Контрагент'                 | 'Взаимодействие (задача)'   | 'Источник события' | 'Объект события'             | 'Структурная единица' | 'Сведения об ошибке или причине отклонения действия' |
