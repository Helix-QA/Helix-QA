...
//		И Я закрываю окно "Согласие на обработку персональных данных (в соответствии с требованиями 152-ФЗ)"
		...
		И из выпадающего списка с именем 'ОтборСтруктурнаяЕдиница' я выбираю точное значение "Фитнес плюс"
...
Сценарий: Я создаю клиента для объединения
	...
	*Заполнение данными 1 клиента	
		...
		И я нажимаю на кнопку с именем 'ВзаимодействиеЗадачаСоздать'
		И я сохраняю навигационную ссылку текущего окна в переменную "$СсылкаКлиент12$"
		И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'
	*Создание 2 клиента
		Дано Я открываю основную форму справочника "Контрагенты"
		И в поле с именем 'Фамилия' я ввожу текст "Клиент объединения"
	...
	*Сохраняю и объединяю клиентов
		...
		Когда открылось окно "Объединение клиентов"
	...
	*Проверка клиента на объединение
		Дано Я открываю навигационную ссылку "$СсылкаКлиент12$"
    	//Тогда элемент формы с именем "ПометкаУдаления" стал равен "Истина"
		И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'
