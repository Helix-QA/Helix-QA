...
//Проверка списания.
	...
	И я жду, что в таблице "Список" количество строк будет "больше" 1 в течении 5 секунд	
	И таблица 'Список' содержит строки по шаблону:
...
//Создание отказа со штрафом.
	...
	Дано я запоминаю значение выражения 'Формат(ТекущаяДата()-3600, "ДЛФ=T")' в переменную "Test"
...
//Проверка списания.
	...
	И таблица 'Список' содержит строки по шаблону:
		| "Занятие"                                                                    | "Количество" | "Регистратор"                                       | "Услуга, сегмент"                                            |
		| "Аренда зала: * $$СегодняДляЗанятия$$, $$УслугаАрендаЗала$$, Фитнес плюс, *" | "1,000"      | "Запись на занятие * от * $$СегодняДляЗанятия$$:00" | "$$УслугаАрендаЗала$$, $$УслугаАрендаЗала$$, Розничная цена" |