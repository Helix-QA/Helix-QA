...
//Проверка окон после запуска.	
	...
	И в дополнение формы с именем 'СтрокаПоиска' я ввожу текст "Фитнес плюс"
...
//Создание кассы АТОЛ УСН.
	...
	И из выпадающего списка с именем 'ВерсияФФДдляККТ' я выбираю точное значение "1.2"
	И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'
//Создание SRP:Юкасса для рекуррентов.
	Дано я открываю основную форму справочника "ПодключаемоеОборудование"
	И из выпадающего списка с именем 'ТипОборудования' я выбираю точное значение "Сервисы рекуррентных платежей"
	И из выпадающего списка с именем 'ДрайверОборудования' я выбираю точное значение "SRP: ЮКасса"
	И я нажимаю кнопку выбора у поля с именем 'Организация'
	И в таблице 'Список' я выбираю текущую строку
	И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'
...
Сценарий: Отключение регламента по триггерным событиям.
Если переменная "ИмяКонфигурации" имеет значение "СалонКрасоты_SPA" Тогда
	Тогда я останавливаю выполнение сценария
	Дано Я открываю основную форму обработки "РегламентныеИФоновыеЗадания"
	И я жду, что в таблице "ТаблицаРегламентныеЗадания" количество строк будет "больше" 0 в течении 40 секунд"
	И в таблице 'ТаблицаРегламентныеЗадания' в дополнение формы с именем 'ТаблицаРегламентныеЗаданияСтрокаПоиска' я ввожу текст "Регистрация триггерных событий"
	И пауза 2
	И в таблице 'ТаблицаРегламентныеЗадания' я выбираю текущую строку
	И я снимаю флаг с именем 'Использование'
	И я нажимаю на кнопку с именем 'ЗаписатьИЗакрыть'
