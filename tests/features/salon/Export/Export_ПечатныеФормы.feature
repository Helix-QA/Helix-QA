...
//полная дата
	Дано Я запоминаю значение выражения '$$ДеньНоябрь$$ $$МесяцНоябрьТекст$$ $$ГодНоябрь$$ г.' в переменную "$$ДатаC01$$"
...
//Создаю 30 / 30
	...
	И я запоминаю значение выражения 'Формат($ДнейВМесяце$, "ЧГ=0") + ",00"' в переменную "$$ТекущКонецМесяц,00$$"
