...
Сценарий: Я создаю точное данные по звонку
	Дано Я запоминаю значение выражения 'Формат(ТекущаяДата()-3600, "ДФ=ЧЧ:мм")' в переменную "$$ТекЧасМин$$"
	...
	"""bsl
	// Берём номер из контекста и чистим до голых цифр
	...
	// Если вдруг 10 цифр — добавляем 7 в начало
	Если СтрДлина(ЧистыйНомер) = 10 Тогда
		ЧистыйНомер = "7" + ЧистыйНомер;
	КонецЕсли;

	// Теперь жёстко режем по позициям (11 цифр гарантированы)
	Часть1 = "+7";
	Часть2 = "(" + Сред(ЧистыйНомер, 2, 3) + ")";     // 910 → (910)
	Часть3 = Сред(ЧистыйНомер, 5, 7);                 // 4785425

	// Кидаем в контекст Vanessa
	Контекст.Вставить("Телефон_Часть1", Часть1);   // +7
	Контекст.Вставить("Телефон_Часть2", Часть2);   // (910)
	Контекст.Вставить("Телефон_Часть3", Часть3);   // 4785425
	"""
	...
	И Я запоминаю значение выражения 'Формат($Телефон_Часть1$, "ЧГ=0") + " " + Формат($Телефон_Часть2$, "ЧГ=0") + " " + Формат($Телефон_Часть3$, "ЧГ=0")' в переменную "$$КрасивыйТелефонКлиента1$$"
