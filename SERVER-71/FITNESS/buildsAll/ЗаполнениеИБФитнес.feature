...
Контекст:
	Дано Я подключаю клиент тестирования с параметрами:
		...
		| 'udd' | ''        | 'Тонкий'      | '48 000' | 'File="D:\Начальное заполнение_удалить потом";' | 'Админ' | ''       | ''                      | ''                                        |
...
//Проверка окон после запуска.	
	...
	И открылось окно "Аналитика фитнес-индустрии"				
//Проверка новой структурной единицы.
	Дано я открываю основную форму списка справочника "СтруктурныеЕдиницы"
	И в таблице "Список" я выбираю текущую строку
	И я сохраняю навигационную ссылку текущего окна в переменную "$$СсылкаНаСтруктурку$$"
...
Сценарий: Создание и настройка подключаемого оборудования.
	...
	И Я устанавливаю в константу "РежимТестированияККТ" значение "Истина"
//Создание подключаемого оборудования.
	...
	И в таблице "Список" я выбираю текущую строку
	И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'
	И открылось окно "Оборудование: \'АТОЛ:ККТ с передачей данных в ОФД 10.Х (ФФД 1.2)\' *"
	...
	И я перехожу к закладке с именем 'ДрайверИнтеграционныйКомпонент'
	И я нажимаю на кнопку с именем 'ЗаписатьИЗакрыть'
	И я закрываю все окна клиентского приложения
	//Создание Life-Pay.
	И В командном интерфейсе я выбираю "Настройки" "Подключаемое оборудование"
...
//Добавление в структурную единицу касс.
	...
	И в таблице 'Список' я перехожу к строке:
		...
		| "ОнлайнКасса"  | "Основная ораганизация" |
	И в таблице 'Список' я выбираю текущую строку
	И я нажимаю на кнопку с именем 'КассыДляОплатыДобавитьКассу'
	И в таблице 'Список' я перехожу к строке:
		| "Наименование" | "Организация"           |
		| "LifePay"      | "Основная ораганизация" |
	И в таблице 'Список' я выбираю текущую строку
	И я нажимаю на кнопку с именем 'КассыДляОплатыДобавитьКассу'
	И в таблице 'Список' я перехожу к строке:
		| "Наименование" | "Организация"           |
		| "АтолУСН"      | "Основная ораганизация" |
	И в таблице 'Список' я выбираю текущую строку
	И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'
