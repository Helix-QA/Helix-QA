...
//Проверка кол-ва услуг после продажи членства.	
	Дано Я открываю основную форму списка регистра накопления "Услуги"
	...
	И таблица 'Список' стала равной по шаблону:
		...
		| '1' | 'Членство "$$ЧленствоЧПУ$$" * от *.*.* *:* ' | '$$УслугаПерсональная$$' | '1,000'      | 'Фитнес плюс'         |
		| '2' | 'Членство "$$ЧленствоЧПУ$$" * от *.*.* *:* ' | '$$УслугаГрупповая$$'    | '1,000'      | 'Фитнес плюс'         |
...
//Проверка по регистру добавились ли услуги.
	Когда В панели открытых я выбираю "Реализованные услуги членств (пакетов услуг) и услуги"
	Тогда открылось окно "Реализованные услуги членств (пакетов услуг) и услуги"
...
//Создание занятия.	
	...
	Попытка
		И элемент формы с именем "ПолеОснованияОплаты_0" стал равен "Пакет услуг: \"$$ПробныйПакет$$\" от $ТекущаяДата$"
	Исключение
		Если элемент формы с именем "ПолеОснованияОплаты_0" стал равен "Членство: \"$$ЧленствоЧПУ$$\" от $ТекущаяДата$" Тогда
			Тогда я вызываю исключение "Некорректное основание для записи"
