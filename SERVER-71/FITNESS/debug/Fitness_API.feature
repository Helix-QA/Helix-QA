...
Сценарий: 01. Создание условий для проверки	
	...
	Дано я открываю внешнюю обработку или отчет "D:\Git репозиторий\Helix-QA\tests\epf\_Universal_nyy_redaktor_rekvizitov_8_2_upravlyaemyy_interfeys_2.epf" (Расширение)	
...
Сценарий: 02. Проверка методов
	...
	Дано я открываю внешнюю обработку или отчет "D:\Git репозиторий\Helix-QA\tests\epf\Test_Api.epf" (Расширение)
	...
	Тогда таблица 'РезультатДерево' содержит строки по шаблону:
		| 'Ключ'                 | 'Значение'                             |
		| 'result'               | 'Да'                                   |
		| 'id'                   | '4a6c88a9-9dd1-11ef-982f-00e04c811603' |
		| 'title'                | 'Фитнес плюс'                          |
	И в таблице "РезультатДерево" я перехожу к строке
		| 'Ключ' |
		| 'id'   |
...
/////МЕТОД "comments" выполняется с ошибкой - ТИКЕТ 000667510.
	...
	И в поле с именем 'start_date' я ввожу текст "$$Сегодня$$"
