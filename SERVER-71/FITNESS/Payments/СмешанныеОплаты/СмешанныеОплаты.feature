...
// Проверка на наличие сотрудника
	Если В базе отсутствует пользователь ИБ "МенеджерОП_Тест" Тогда
		И СО_Я создаю менеджера
// Настройки менеджера 
	Если существует переменная "$$СсылкаМенеджер$$" Тогда
		И Я удаляю пользователя "МенеджерОП_Тест"
	...
	Если не существует переменная "$$СсылкаМенеджер$$" Тогда
		И Я удаляю пользователя "МенеджерОП_Тест"
		...
		И я устанавливаю флаг с именем 'ДоступКИнформационнойБазеРазрешен'
		И я нажимаю на кнопку с именем '_СвойстваПользователяИБ'
		И в поле с именем 'ПользовательИБИмя2' я ввожу текст "МенеджерОП_Тест"	
		И из выпадающего списка с именем 'РольПользователя' я выбираю точное значение "Менеджер отдела продаж"
		И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'		
...
Сценарий: 02. Продажи/оплаты
	...
	И я подключаю TestClient "Этот менеджер" логин "МенеджерОП_Тест" пароль ""
	...
	* Проверка оплаты с бонусного счета
		...
		И пауза 1
		Если элемент формы с именем 'КнопкаВидыОплатПрименитьеСуммуСтрока_2x0' присутствует на форме Тогда
	...
	* Проверка смешанных оплат (наличные и ЛС)
		...
		Если элемент формы с именем 'КнопкаВидыОплатПрименитьеСуммуСтрока_1x0' присутствует на форме Тогда
